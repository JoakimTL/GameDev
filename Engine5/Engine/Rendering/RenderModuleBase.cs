...
public abstract class RenderModule : ModuleBase<IRenderService>, ISystem, IInitializable, IUpdateable, IDisposable
{
    ...
    {
        Thread.CurrentThread.Name = "OpenGl Context Thread";
        Gl.Initialize();
        Get<GlDebugMessageService>().BindErrorCallback();
        GlfwUtilities.Init();
        //TODO: Fetch window saved settings
        var window = Get<WindowService>().Create(new WindowSettings());
    ...
    protected virtual void OnInitialize(Window window) { }
