...
public class MouseInputEventListener : Identifiable
{
	...
	internal MouseInputEventListener(Window window)
    {
        _window = window;
    ...
    private void OnEnter( nint winPtr, bool enter ) {
		...
		if ( _window.Pointer != winPtr ) {
			this.LogWarning( $"{nameof( OnEnter )} {nameof( winPtr )} parameter [{winPtr}] does not match [{_window.Pointer}]!" );
			return;
		}
#endif
    ...
    private void OnButton(nint winPtr, MouseButton button, InputState state, ModifierKeys modifiers)
    {
        ...
        if (_window.Pointer != winPtr)
        {
            this.LogWarning( $"{nameof(OnButton)} {nameof(winPtr)} parameter [{winPtr}] does not match [{_window.Pointer}]!");
            return;
        }
#endif
    ...
    private void OnCursor(nint winPtr, double x, double y)
    {
        ...
        if (_window.Pointer != winPtr)
        {
            this.LogWarning( $"{nameof(OnCursor)} {nameof(winPtr)} parameter [{winPtr}] does not match [{_window.Pointer}]!");
            return;
        }
#endif
    ...
    private void OnScroll(nint winPtr, double x, double y)
    {
        ...
        if (_window.Pointer != winPtr)
        {
            this.LogWarning( $"{nameof(OnScroll)} {nameof(winPtr)} parameter [{winPtr}] does not match [{_window.Pointer}]!");
            return;
        }
#endif
