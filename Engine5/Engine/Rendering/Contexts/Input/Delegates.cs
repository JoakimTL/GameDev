...
//TIMED MOUSE
public delegate void TimedMouseButtonHandler( float time, ButtonEventState state );
public delegate void TimedMouseScrollHandler( float time, Point2d scroll );
public delegate void TimedMouseStateChangeHandler( float time, bool state );
public delegate void TimedMouseMoveHandler( float time, MousePointerState state, bool lockState );
//KEYBOARD
