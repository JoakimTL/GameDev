...
public sealed class Transform2Component : ComponentBase, ICustomizedSerializable
{
	...
	protected override void OnOwnerChanged() => Transform.SetParent(Owner?.Parent?.Get<Transform2Component>()?.Transform, true);
	protected override void OnOwnerParentChanged(Entity owner, Entity? parent) => Transform.SetParent(parent?.Get<Transform2Component>()?.Transform, true);
