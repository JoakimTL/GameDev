...
vec3 solid(vec3 lightingData){
	...
	vec4 pDiff = texture(uTexShadowDiffuse[chosenCascade], lUV);

	return light * visibility * (pDiff.rgb * pDiff.a);
}
...
void main(void) {
	...
	outColor = vec4(solid(lightingData), 1.0);
}