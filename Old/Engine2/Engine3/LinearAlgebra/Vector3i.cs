...
namespace Engine.LinearAlgebra {
	...
	public struct Vector3i : System.IEquatable<Vector3i> {
		...
		public Vector3b AsByteUnchecked => new Vector3b( unchecked((byte) X), unchecked((byte) Y), unchecked((byte) Z) );
		...
		public static Vector3i Min( Vector3i a, Vector3i b ) {
			return new Vector3i( System.Math.Min( a.X, b.X ), System.Math.Min( a.Y, b.Y ), System.Math.Min( a.Z, b.Z ) );
		}

		public static Vector3i Max( Vector3i a, Vector3i b ) {
			return new Vector3i( System.Math.Max( a.X, b.X ), System.Math.Max( a.Y, b.Y ), System.Math.Max( a.Z, b.Z ) );
		}
		#endregion
		...
		public override int GetHashCode() {
			uint x = (uint) X.GetHashCode();
			uint y = (uint) Y.GetHashCode();
			uint z = (uint) Z.GetHashCode();
			uint xx = x << 24 ^ x << 8 | x >> 8 ^ x >> 24;
			uint yy = y << 24 ^ y << 8 | y >> 8 ^ y >> 24;
			uint zz = z << 24 ^ z << 8 | z >> 8 ^ z >> 24;
			return (int) ( ( ( x - yy & ~zz ) + ( y - zz & ~xx ) + ( z - xx & ~yy ) ) ^ ( ( x + yy & ~zz ) - ( y + zz & ~xx ) - ( z + xx & ~yy ) ) );
		}
		...
		public static implicit operator Vector3i( (int, int, int) a ) {
			return new Vector3i( a.Item1, a.Item2, a.Item3 );
		}
