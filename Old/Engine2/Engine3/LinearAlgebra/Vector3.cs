...
namespace Engine.LinearAlgebra {
	...
	public struct Vector3 : System.IEquatable<Vector3> {
		...
		public Vector3i IntSign => new Vector3i( System.Math.Sign( X ), System.Math.Sign( Y ), System.Math.Sign( Z ) );
		...
		public static void TransformGraphics( ref Vector3 vec, ref Matrix4 mat, out Vector3 result, out bool behind ) {
			Vector4 v4 = new Vector4( vec.X, vec.Y, vec.Z, 1.0f );
			Vector4.Transform( ref v4, ref mat, out v4 );
			behind = v4.Z < 0;
			result = v4.XYZ / v4.W;
		...
		public static Vector3 TransformGraphics( ref Vector3 vec, ref Matrix4 mat, out bool behind ) {
			TransformGraphics( ref vec, ref mat, out Vector3 result, out behind );
			return result;
		}

		public static Vector3 TransformGraphics( Vector3 vec, Matrix4 mat, out bool behind ) {
			TransformGraphics( ref vec, ref mat, out Vector3 result, out behind );
			return result;
		}
		#endregion
		...
		#region Barycentric
		public static void ToBarycentricTetrahedron( ref Vector3 a, ref Vector3 b, ref Vector3 c, ref Vector3 d, ref Vector3 p, out Vector4 barycentric ) {
			Vector3 vap = p - a;
			Vector3 vbp = p - b;
			...
			float v6 = 1f / ScalarTripleProduct( vab, vac, vad );

			barycentric = new Vector4( va6 * v6, vb6 * v6, vc6 * v6, vd6 * v6 );
		}
		public static Vector4 ToBarycentricTetrahedron( Vector3 a, Vector3 b, Vector3 c, Vector3 d, Vector3 p ) {
			ToBarycentricTetrahedron( ref a, ref b, ref c, ref d, ref p, out Vector4 barycentric );
			return barycentric;
		}

		//Taken from https://vccimaging.org/Publications/Heidrich2005CBP/Heidrich2005CBP.pdf
		/// <summary>
		/// Returns the barycentric coordinates of a point projected onto the triangle
		/// </summary>
		/// <param name="a"></param>
		/// <param name="b"></param>
		/// <param name="c"></param>
		/// <param name="p"></param>
		/// <returns></returns>
		public static Vector3 ToBartcentricProjectedTriangle( Vector3 a, Vector3 b, Vector3 c, Vector3 p ) {
			Vector3 ab = b - a;
			Vector3 ac = c - a;
			Vector3 n = Cross( ab, ac );
			float oneOver4ASquared = 1.0f / Dot( n, n );
			Vector3 w = p - a;
			Vector3 r = new Vector3( 0, Dot( Cross( w, ac ), n ) * oneOver4ASquared, Dot( Cross( ab, w ), n ) * oneOver4ASquared );
			r.X = 1 - r.Y - r.Z;
			return r;
		}
		#endregion

		public static Vector3 Cubify( Vector3 v ) {
			float l = Math.Abs( Math.Min( Math.Min( 1f / v.X, 1f / v.Y ), 1f / v.Z ) );
			return v * l;
		}
		...
		public static implicit operator Vector3( (float, float, float) a ) {
