...
namespace Engine.Graphics.Objects.Default.Particles.D3 {
	public class Animation3Manager {
		private readonly Particle3 p;
		...
		public Animation3Manager( Particle3 p ) {
			this.p = p;
		...
		public void Update() {
			...
			Vector4b sourceGlow = 0;
			if( !( source is null ) ) {
				sourceColor = source.targetDiffuse;
				sourceGlow = source.targetGlow;
			}
			p.Data.Diffuse = target.targetDiffuse * interp + sourceColor * ( 1 - interp );
			p.Data.Glow = target.targetGlow * interp + sourceGlow * ( 1 - interp );
		}
