...
namespace Engine.Graphics.Objects.Default.Materials {
	public class PBRMaterial : Material{
		...
		public PBRMaterial( string name, Texture diffuse, Texture normal, Texture lighting, Texture glow ) : base( name ) {
			Properties = new Property();
			shaderBind = PBRApplication;

			AddTexture( OpenGL.TextureUnit.Texture0, diffuse );
			AddTexture( OpenGL.TextureUnit.Texture1, normal );
			AddTexture( OpenGL.TextureUnit.Texture2, lighting );
			AddTexture( OpenGL.TextureUnit.Texture3, glow );
		}
