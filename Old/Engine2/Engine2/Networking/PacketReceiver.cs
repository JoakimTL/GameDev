	...
	public class PacketReceiver {
		...
		private void Receive() {
			try {
				...
				MemLib.Mem.Logs.Routine.WriteLine( "Listening for " + socket.ProtocolType + " messages!" );
				Running = true;
				while( MemLib.Mem.Threads.Running && Running ) {
					...
					while( readData.Data.Count < readData.IntendedLength && activeData.Count > 0 ) {
						...
						if( readData.Data.Count == readData.IntendedLength && readData.Data.Count > 0 ) {
							//Packet is done!
							Packet p = PacketType.CreatePacket( address, readData.Data );
							readData.Reset( 0 );
			...
			MemLib.Mem.Logs.Routine.WriteLine( "Stopped listening for " + socket.ProtocolType + " messages!" );
		}
