...
namespace Engine.LMath {
	...
	public struct Quaternion : System.IEquatable<Quaternion>, ILANormalizable<Quaternion>, ILAMeasurable {
		...
		public static Quaternion LookTowards( Vector3 forward, Vector3 up ) {
			...
			if( trace > 0.0 ) {
				...
				if( R.X > U.Y && R.X > F.Z ) {
					float s = 2.0f * (float) Math.Sqrt( 1.0 + R.X - U.Y - F.Z );
					q.W = ( U.Z - F.Y ) / s;
					q.X = 0.25f * s;
					q.Y = ( U.X + R.Y ) / s;
					q.Z = ( F.X + R.Z ) / s;
				} else if( U.Y > F.Z ) {
					float s = 2.0f * (float) Math.Sqrt( 1.0 + U.Y - R.X - F.Z );
					q.W = ( F.X - R.Z ) / s;
					q.X = ( U.X + R.Y ) / s;
					q.Y = 0.25f * s;
					q.Z = ( F.Y + U.Z ) / s;
				} else {
					float s = 2.0f * (float) Math.Sqrt( 1.0 + F.Z - R.X - U.Y );
					q.W = ( R.Y - U.X ) / s;
					q.X = ( F.X + R.Z ) / s;
					q.Y = ( F.Y + U.Z ) / s;
					q.Z = 0.25f * s;
