...
[Engine.Processing.Do<IGlobeGenerationProcessingStep>.After<NodePlateEdgeDistanceCalculationStep>]
public sealed class TectonicLandscapeGenerationStep( TectonicGenerationParameters parameters ) : TectonicGlobeGenerationProcessingStepBase( parameters ) {
	public override void Process( Globe globe ) {
		...
		foreach (SphericalVoronoiRegion plate in sphericalRegions) {
			...
			float wPassiveRad = 220f / (float) (globe.Radius / 1000);
		...
		float lowestBaseElevation = globe.Nodes.Min( n => n.GetStateOrThrow<NodeTectonicLandscapeState>().BaseElevation );
		float highestBaseElevation = globe.Nodes.Max( n => n.GetStateOrThrow<NodeTectonicLandscapeState>().BaseElevation );
