...
//	//	builder.Debug_Arrow = this.TangentialWind.FromNumerics<float>() * 0.1f;//(DownslopeNeighbour is not null) ? (DownslopeNeighbour.Center - Face.Center).Normalize<Vector3<float>, float>() * 0.33f : Vector3<float>.Zero;//Wind.Normalize<Vector3<float>, float>();
//	//	builder.Debug_Color = (this.SpecificHumidity / 0.002f, float.Max( -this.AverageAirTemperature.Celsius, 0 ) / 120, this.Face.IsOcean ? 1 : 0, 1);
//	//}
//}
