...
//using OldGen.World.GameplayState;
...
//	public static (Globe, GlobeGeneratorParameterBase) GenerateGlobe( Type globeType, Type parametersType, object parameters, Type[]? stepsToIgnore = null ) {
//		Type[] genericArgs = [ globeType, parametersType ];
//		MethodInfo openGen = typeof( GlobeGenerator ).GetMethod( nameof( Generate ), BindingFlags.Public | BindingFlags.Static ) ?? throw new InvalidOperationException( "Couldn’t find Generate method" );
//		MethodInfo genMethod = openGen.MakeGenericMethod( genericArgs );
//		Globe globe = (Globe) (genMethod.Invoke( null, [ parameters, stepsToIgnore ] ) ?? throw new InvalidOperationException( "Generate returned null" ));

//		GlobeGeneratorParameterBase globeParameters = parameters as GlobeGeneratorParameterBase ?? throw new InvalidOperationException( $"Parameters must be derived from {nameof( GlobeGeneratorParameterBase )}" );
//		return (globe, globeParameters);
//	}
...
//		int messageInterval = Math.Max( 1, loopCount / 100 ); // Publish progress every 1% of the iterations
//		int loopsTilMessage = messageInterval;
//		for (int i = 0; i < loopCount; i++) {
//			--loopsTilMessage;
//			if (loopsTilMessage == 0) { 
//				loopsTilMessage = messageInterval;
...
//		foreach ((FaceBase generated, GameplayState.Face.Builder builder) in faceBuilders) {
...
//		for (int x = 0; x < clustersPerAxis; x++) 			for (int y = 0; y < clustersPerAxis; y++) 				for (int z = 0; z < clustersPerAxis; z++) {
