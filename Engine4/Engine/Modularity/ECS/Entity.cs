...
public class Entity : Identifiable, IUpdateable {
	...
	private readonly HashSet<IUpdateable> _updateableComponents;
	...
	public Entity( string name, Guid guid, ulong owner ) : base( name, guid ) {
		...
		this._updateableComponents = new();
		this._componentRefs = new();
		this.Owner = owner;
		this.Alive = true;
		this.CreationTime = Clock32.StartupTime;
	}

	public Entity( string name, ulong owner ) : this( name, Guid.NewGuid(), owner ) { }
	...
	public void Update( float time, float deltaTime ) {
		foreach ( IUpdateable? component in this._updateableComponents )
			if ( component.Active )
	...
	public T? AddComponent<T>( T? component ) where T : Component {
		...
		Type type = Resources.GlobalService<ComponentTypeRegistry>().GetRegisteredAs( originalType );
		...
		if ( component is IUpdateable uComponent )
	...
	public Component? RemoveComponent( Type type ) {
		if ( this._componentRefs.Remove( Resources.GlobalService<ComponentTypeRegistry>().GetRegisteredAs( type ), out Component? component ) ) {
			this._components.Remove( component );
			Type refType = Resources.GlobalService<ComponentTypeRegistry>().GetRegisteredAs( type );
			if ( refType != type )
	...
	public static byte[]? Serialize( Entity e ) {
		...
		foreach ( ISerializableComponent component in e._components.OfType<ISerializableComponent>() ) {
