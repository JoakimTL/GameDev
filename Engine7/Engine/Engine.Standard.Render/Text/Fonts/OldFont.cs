...
public sealed class OldFont {
	...
	private Dictionary<char, IOldGlyph> _glyphs = [];
	private IOldGlyph _missingGlyph;

	internal unsafe OldFont( string path ) {
		// Load font
		this.Path = path;
	...
	public IOldGlyph this[ char c ] => this._glyphs.TryGetValue( c, out IOldGlyph? glyph ) ? glyph : this._missingGlyph;
	...
	private static unsafe IOldGlyph ReadGlyph( byte* srcPtr, Span<uint> glyphLocations, GlyphMap mapping, GlyphMap[] mappings, ushort unitsPerEm ) {
	...
	private static unsafe OldFontCompoundGlyph ReadCompoundGlyph( byte* srcPtr, Span<uint> glyphLocations, nint loc, FontGlyphHeader header, GlyphMap mapping, GlyphMap[] mappings, ushort unitsPerEm ) {
		OldFontCompoundGlyph compoundGlyph = new( header, mapping );

		while (true) {
			(IOldGlyph? componentGlyph, bool hasMoreGlyphs) = ReadNextComponentGlyph( srcPtr, glyphLocations, ref loc, mapping, mappings, unitsPerEm );
	...
	private static unsafe (IOldGlyph? componentGlyph, bool hasMoreGlyphs) ReadNextComponentGlyph( byte* srcPtr, Span<uint> glyphLocations, ref nint loc, GlyphMap mapping, GlyphMap[] mappings, ushort unitsPerEm ) {
		...
		IOldGlyph componentGlyph = ReadGlyph( srcPtr, glyphLocations, new( glyphIndex, 65535 ), mappings, unitsPerEm );
		if (componentGlyph is OldFontGlyph simpleGlyph)
	...
	private static unsafe OldFontGlyph ReadSingleGlyph( byte* srcPtr, nint offset, FontGlyphHeader header, GlyphMap mapping, ushort unitsPerEm ) {
		...
		return new OldFontGlyph( header, mapping, points, endPointsOfContours, unitsPerEm );
