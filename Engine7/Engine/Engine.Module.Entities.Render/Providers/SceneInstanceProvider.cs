...
public sealed class SceneInstanceProvider( SceneService sceneService, CompositeVertexArrayProvider compositeVertexArrayProvider, ShaderBundleProvider shaderBundleProvider ) {
	...
	public T RequestSceneInstance<T>( string sceneName, uint layer ) where T : SceneInstanceBase, new() => this._sceneService.GetScene( sceneName ).CreateInstance<T>( layer, true );
