...
public abstract class TileDataBase {
	public TileDataModel DataModel { get; private set; }

	protected TileDataBase() {
		DataModel = null!;
	}

	internal void SetDataModel( TileDataModel dataModel ) {
		if (DataModel is not null)
			throw new InvalidOperationException( $"{nameof( DataModel )} already set." );
		DataModel = dataModel;
	}
}
