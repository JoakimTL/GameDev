...
public readonly struct Vector4<TScalar>( TScalar x, TScalar y, TScalar z, TScalar w ) :
	...
	public bool GreaterThanEntrywise( in Vector4<TScalar> other ) => this.X > other.X && Y > other.Y && Z > other.Z && W > other.W;
	public bool GreaterThanOrEqualEntrywise( in Vector4<TScalar> other ) => this.X >= other.X && Y >= other.Y && Z >= other.Z && W >= other.W;
	...
	public override string ToString() 
		=> string.Create( CultureInfo.InvariantCulture, 
			$"[{X:#,##0.###}X {Y.SignCharacter()} {TScalar.Abs(Y):#,##0.###}Y {Z.SignCharacter()} {TScalar.Abs(Z):#,##0.###}Z {W.SignCharacter()} {TScalar.Abs(W):#,##0.###}W]" );
}