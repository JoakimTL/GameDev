...
public readonly struct Bivector4<TScalar>( TScalar yz, TScalar zx, TScalar xy, TScalar yw, TScalar zw, TScalar xw ) :
	...
	public override string ToString()
		=> string.Create( CultureInfo.InvariantCulture,
			$"[{YZ:#,##0.###}YZ {ZX.SignCharacter()} {TScalar.Abs( ZX ):#,##0.###}ZX {XY.SignCharacter()} {TScalar.Abs( XY ):#,##0.###}XY {YW.SignCharacter()} {TScalar.Abs( YW ):#,##0.###}YW {ZW.SignCharacter()} {TScalar.Abs( ZW ):#,##0.###}ZW {XW.SignCharacter()} {TScalar.Abs( XW ):#,##0.###}XW]" );
}
