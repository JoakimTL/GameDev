...
public static class Vector3Ops {
	...
	public static bool TryNormalize<T>( in this Vector3<T> l, out Vector3<T> result ) where T : unmanaged, IFloatingPointIeee754<T> {
		T magnitude = l.Magnitude();
		if (magnitude == T.Zero) {
			result = default;
			return false;
		}
		result = l.ScalarDivide( magnitude );
		return true;
	}
