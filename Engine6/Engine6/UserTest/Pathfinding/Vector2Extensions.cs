...
public static class Vector2Extensions {
	...
	where TScalar :
		...
		if (order.Length != vertices.Length)
			return Log.WarningThenReturn( "Order span must be the same length as the vertices span.", 0u );
		if (vertices.Length < 3)
			return Log.WarningThenReturn( "Must have more than two vertices.", 0u );
