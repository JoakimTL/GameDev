...
public sealed class FramebufferState( ContextWarningLog warningLog ) {
	...
	public OglFramebuffer CreateFramebuffer( Vector2ui size ) {
		uint framebufferId = Gl.CreateFramebuffer();
		return new( _warningLog, framebufferId, size );
	}

	public void BindFramebuffer( FramebufferTarget target, OglFramebuffer buffer ) {
		if (buffer.Disposed) {
			_warningLog.LogWarning( $"Trying to bind disposed framebuffer {buffer}" );
			return;
		}
		...
		switch (target) {
			case FramebufferTarget.DrawFramebuffer:
				if (buffer.FramebufferId == _boundDrawBuffer)
					_warningLog.LogWarning( $"Framebuffer {buffer} already bound to draw buffer" );
			...
			case FramebufferTarget.ReadFramebuffer:
				if (buffer.FramebufferId == _boundReadBuffer)
					_warningLog.LogWarning( $"Framebuffer {buffer} already bound to read buffer" );
			...
			case FramebufferTarget.Framebuffer:
				if (buffer.FramebufferId == _boundDrawBuffer && buffer.FramebufferId == _boundReadBuffer)
					_warningLog.LogWarning( $"Framebuffer {buffer} already bound to both draw and read buffer" );
		...
		switch (target) {
			case FramebufferTarget.DrawFramebuffer:
				if (_boundDrawBuffer == 0)
					_warningLog.LogWarning( "Trying to unbind framebuffer as draw buffer, but no framebuffer bound to draw buffer" );
				_boundDrawBuffer = 0;
				Gl.BindFramebuffer( target, 0 );
				break;
			case FramebufferTarget.ReadFramebuffer:
				if (_boundReadBuffer == 0)
					_warningLog.LogWarning( "Trying to unbind framebuffer as read buffer, but no framebuffer bound to read buffer" );
				_boundReadBuffer = 0;
				Gl.BindFramebuffer( target, 0 );
				break;
			case FramebufferTarget.Framebuffer:
				if (_boundDrawBuffer == 0 && _boundReadBuffer == 0)
					_warningLog.LogWarning( "Trying to unbind framebuffer as both draw and read buffer, but no framebuffer bound to either buffer" );
	...
	public void BlitToFrameBuffer( OglFramebuffer source, OglFramebuffer destination, ClearBufferMask mask, BlitFramebufferFilter filter ) {
		...
		BlitFramebuffer( (0, 0), source.Size.ToSigned(), (0, 0), destination.Size.ToSigned(), mask, filter );
	...
	public void BlitToScreen( OglFramebuffer source, OglWindow window, ClearBufferMask mask, BlitFramebufferFilter filter ) {
		BindFramebuffer( FramebufferTarget.ReadFramebuffer, source );
		UnbindFramebuffer( FramebufferTarget.DrawFramebuffer );
		BlitFramebuffer( (0, 0), source.Size.ToSigned(), (0, 0), window.Size.ToSigned(), mask, filter );
