...
public abstract class OglTextureBase<T> : OglObjectBase where T : struct {
	...
	protected readonly IReadOnlyList<Vector2ui> _levels;
	...
	/// </summary>
	public Vector2ui Level0 => _levels[ 0 ];
	public bool Resident { get; private set; }

	protected override string DisplayName => $"TEX#{(Nickname.Length > 0 ? $"{Nickname}:" : "")}{TextureID}";

	public OglTextureBase( ContextWarningLog warningLog, TextureTarget target, Vector2ui level0, T metadata, params (TextureParameterName, int)[] parameters ) {
		if (level0.X == 0 || level0.Y == 0)
			throw new OpenGlArgumentException( "Texture cannot be size zero on any axis", nameof( level0 ) );
	...
	protected abstract IReadOnlyList<Vector2ui> GetLevels( Vector2ui level0, T metadata );

	internal ulong Handle => _handle;

	public Vector2ui GetLevel( uint level ) {
	...
	public void SetPixels( PixelFormat format, PixelType pixelType, nint ptr, uint level = 0 ) {
		...
		Vector2ui mm = _levels[ (int) level ];
		Gl.TextureSubImage2D( TextureID, (int) level, 0, 0, (int) mm.X, (int) mm.Y, format, pixelType, ptr );
	}
	...
	public void SetPixelsCompressed( PixelFormat format, int size, nint ptr, uint level = 0 ) {
		...
		Vector2ui mm = _levels[ (int) level ];
		Gl.CompressedTextureSubImage2D( TextureID, (int) level, 0, 0, (int) mm.X, (int) mm.Y, format, size, ptr );
	}
