...
public abstract class OglBufferBase : OglObjectBase {

	public readonly BufferUsage Usage;
	public readonly uint BufferId;
	public uint LengthBytes { get; private set; }

	protected override string DisplayName => $"BUF#{(Nickname.Length > 0 ? $"{Nickname}:" : "")}{BufferId}[{(LengthBytes / 1024d):N2}KiB]";
	...
	protected void SetSize( uint size ) {
		...
		LengthBytes = size;
		Gl.NamedBufferData( BufferId, size, IntPtr.Zero, Usage );
			...
			if (_fragmented) {
				...
				if (numberOfSegments > 1) {
					...
					for (int i = 0; i < numberOfSegments; i++) {
						...
						//Does not start at the current allocation offset, we need to copy the segment to the new offset.
