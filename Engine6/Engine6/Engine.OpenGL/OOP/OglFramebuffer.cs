...
public sealed class OglFramebuffer : OglObjectBase {
	...
	public OglFramebuffer( ContextWarningLog warningLog, uint framebufferId, Vector2i size ) {
		if (size.X <= 0 || size.Y <= 0)
			throw new OpenGlArgumentException( $"{DisplayName} must have positive non-zero size on both axis", nameof( size ) );
	...
	public void Resize( Vector2i newSize ) {
		if (newSize.X <= 0 || newSize.Y <= 0)
			throw new OpenGlArgumentException( $"{DisplayName} must have positive non-zero size on both axis", nameof( newSize ) );
		if (newSize == Size)
